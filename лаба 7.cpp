...
void deleteNode(Grid*& root, int Key) {
    // базовый случай: если дерево пусто, то ничего не делаем
    if (root == nullptr) {
        return;
    }

    // если ключ меньше, чем ключ корня, то рекурсивно ищем в левом поддереве
    if (Key < root->Key) {
        deleteNode(root->left, Key);
    }
    // если ключ больше, чем ключ корня, то рекурсивно ищем в правом поддереве
    else if (Key > root->Key) {
        deleteNode(root->right, Key);
    }
    // если нашли вершину с заданным ключом
    else {
        // случай 1: вершина не имеет детей
        if (root->left == nullptr && root->right == nullptr) {
            delete root;
            root = nullptr;
        }
        // случай 2: вершина имеет только одного ребенка
        ...
        // случай 3: вершина имеет двух детей
        else {
            Grid* temp = root->right;
...
int main() {
    ...
    while (a)
    {
        cout << "1.Создать\n2.Добавить\n3.Просмотреть\n4.Сбалансировать\n5.Найти по ключу\n6. Удалить элемент с введенным ключем\n7.Удалить по ключу\n8.Выход\n" << endl;
        cout << "Число: " << endl;
        CheckInp(&a);
        switch (a)
        {
